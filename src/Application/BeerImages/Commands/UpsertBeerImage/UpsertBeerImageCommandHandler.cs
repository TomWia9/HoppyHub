...
public class UpsertBeerImageCommandHandler : IRequestHandler<UpsertBeerImageCommand, string>
{
    ...
    public UpsertBeerImageCommandHandler(IApplicationDbContext context, IBeersService beersService)
    {
        _context = context;
    ...
    public async Task<string> Handle(UpsertBeerImageCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var imageUri = await _beersService.UploadBeerImageAsync(request.Image!, beer.BreweryId, request.BeerId);
