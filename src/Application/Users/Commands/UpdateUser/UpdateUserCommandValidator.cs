...
public class UpdateUserCommandValidator : AbstractValidator<UpdateUserCommand>
{
    ...
    public UpdateUserCommandValidator(ICurrentUserService currentUserService)
    {
        ...
        RuleFor(x => x.CurrentPassword)
            .NotEmpty().When(x => !string.IsNullOrEmpty(x.NewPassword) && !currentUserService.AdministratorAccess, ApplyConditionTo.CurrentValidator)
            .MaximumLength(256);

        RuleFor(x => x.NewPassword)
            .NotEmpty().When(x => !string.IsNullOrEmpty(x.CurrentPassword) && !currentUserService.AdministratorAccess, ApplyConditionTo.CurrentValidator)
