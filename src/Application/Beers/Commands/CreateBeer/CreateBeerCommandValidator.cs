...
public class CreateBeerCommandValidator : AbstractValidator<CreateBeerCommand>
{
    ...
    public CreateBeerCommandValidator(IApplicationDbContext context)
    {
        ...
        RuleFor(x => x.Name).NotEmpty().MaximumLength(200)
            .MustAsync((beer, name, cancellationToken) =>
                BeUniqueNameWithinBrewery(name, beer.Brewery, cancellationToken))
    ...
    /// <param name="brewery">The brewery name</param>
    /// <param name="cancellationToken">The cancellation token</param>
    private async Task<bool> BeUniqueNameWithinBrewery(string name, string? brewery,
        CancellationToken cancellationToken)
    {
        return await _context.Beers.Where(x => x.Brewery == brewery)
